...
if "%opcion%"=="5" goto :ACTIVACION_MAS
if "%opcion%"=="6" exit
goto :MENU_PRINCIPAL
...
echo ========================================================
echo.
echo    [1] APLICAR OPTIMIZACION (MAX RENDIMIENTO)
echo        - Boot: Activa TODOS los procesadores (%NUMBER_OF_PROCESSORS%).
echo        - Visual: Desactiva animaciones y sombras.
echo        - Ajusta Windows para "Mejor Rendimiento".
echo.
...
:: Establece "Ajustar para obtener el mejor rendimiento"
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects" /v VisualFXSetting /t REG_DWORD /d 2 /f >nul 2>&1

:: Desactiva arrastre completo de ventanas, sombras y suavizado
...
:: Reiniciar explorer para efectos visuales
taskkill /f /im explorer.exe >nul 2>&1
start explorer.exe
echo.
echo [EXITO] Optimizacion aplicada.
echo         NOTA: Reinicia el equipo para que el cambio de
echo         procesadores surta efecto total.
echo.
pause
goto :MENU_PRINCIPAL
...
:: Establece "Dejar que Windows elija" (Valor 0 o 1 segun version, 3 es personalizar)
...
:: ==========================================
:: SECCION 5: ACTIVACION (MAS)
:: ==========================================
:ACTIVACION_MAS
cls
echo.
echo ======================================================
echo    INICIANDO MICROSOFT ACTIVATION SCRIPTS (MAS)
echo ======================================================
echo.
echo [1/2] Lanzando PowerShell como Administrador...
echo.
echo [INFO] Se abrira una nueva ventana azul.
echo        Espera a que cargue el menu de opciones.
echo.
:: "start" abre una ventana nueva
:: "irm ... | iex" descarga y ejecuta el script en memoria
start powershell -NoProfile -ExecutionPolicy Bypass -Command "irm https://get.activated.win | iex"

echo [2/2] Ejecucion iniciada.
echo.
pause
goto :MENU_PRINCIPAL
