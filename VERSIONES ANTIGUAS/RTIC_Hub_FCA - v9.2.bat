...
:: ==========================================
:: Creado por: Eichhh
:: Versión: 9.2 (Pantalla de inicio)
:: ==========================================
...
echo ======================================================
echo.
echo    No se recibio respuesta del servidor (8.8.8.8).
echo    Es necesario internet para descargar los recursos.
echo.
echo    ¿Deseas configurar tu IP manualmente para intentar
echo    restablecer la conexion?
echo.
echo    [1] SI, Configurar IP Manualmente
echo    [2] NO, Salir del programa
echo.
set /p redfix="Selecciona una opcion: "

if "%redfix%"=="1" (
    :: Restablecemos color y vamos al menu de IP existente
    color 0B
    goto :RED_CONFIG_IP
)

:: Si elige 2 o cualquier otra cosa, sale.
exit
...
:: ==========================================
:: CONFIGURACION DE COLORES Y PANTALLA DE CARGA (ASCII ART)
:: ==========================================
:: Definimos el caracter ESC para usar colores avanzados
for /F "tokens=1,2 delims=#" %%a in ('"prompt #$H#$E# & echo on & for %%b in (1) do rem"') do set "ESC=%%b"

:: Variables de color para facilitar la lectura del codigo
set "C_RTIC=%ESC%[1;37m"
set "C_HUB=%ESC%[38;2;255;165;0m"
set "C_RESET=%ESC%[0m"
...
for /L %%i in (1,1,5) do (
    ...
    echo.
    echo.
    :: LINEA 1: RTIC (Blanco) + HUB (Naranja) + FCA (Blanco Arriba)
    echo    %C_RTIC% ____  _____ ___  ____       %C_HUB% _   _  _   _  ____    %C_RTIC%  FCA
    
    :: LINEA 2 (Corregido el parentesis final: __ ^))
    echo    %C_RTIC%^|  _ \^|_   _^|_ _^|/ ___^|      %C_HUB%^| ^| ^| ^|^| ^| ^| ^|^| __ ^)   %C_RTIC%  ===
    
    :: LINEA 3
    echo    %C_RTIC%^| ^|_) ^| ^| ^|  ^| ^|^| ^|          %C_HUB%^| ^|_^| ^|^| ^| ^| ^|^|  _ \ 
    
    :: LINEA 4 (Corregido el parentesis: ^|_^) )
    echo    %C_RTIC%^|  _ ^<  ^| ^|  ^| ^|^| ^|___       %C_HUB%^|  _  ^|^| ^|_^| ^|^| ^|_^) ^|
    
    :: LINEA 5
    echo    %C_RTIC%^|_^| \_\ ^|_^| ^|___^|\____^|      %C_HUB%^|_^| ^|_^| \___/ ^|____/ %C_RESET%
    echo.
    echo.
    echo             Creado por: Eichhh  ^|  Version: 9.2
    echo.
    echo             [ Cargando modulos... %%i/5 ]
...
:: Se cambia "2" (Estirar) por "10" (Rellenar/Recorte)
reg add "HKCU\Control Panel\Desktop" /v WallpaperStyle /t REG_SZ /d "10" /f >nul 2>&1
:: Se asegura que el mosaico este desactivado
...
:: CAMBIO SOLICITADO: Activamos "Suavizar bordes para las fuentes" (Valor 2)
reg add "HKCU\Control Panel\Desktop" /v FontSmoothing /t REG_SZ /d 2 /f >nul 2>&1

:: Desactiva el resto de efectos pesados (animaciones, arrastre, sombras)
...
echo.
:: Mostramos las interfaces para que el usuario sepa el nombre
netsh interface show interface
echo.
echo    Escribe el nombre EXACTO de la columna "Nombre de interfaz".
echo    (Ejemplo: Ethernet, Wi-Fi, Ethernet 2).
echo.
...
echo.
set /p IP="   - Direccion IP (Ej: 192.168.1.50): "
echo.
echo    [Nota: La mascara comun es 255.255.255.0]
set /p MASCARA="   - Mascara de subred: "
echo.
set /p PUERTA="   - Puerta de enlace (Router): "

echo.
echo [3/5] Configurando Servidores DNS...
echo.
set /p DNS1="   - DNS Preferido (Ej: 8.8.8.8): "
set /p DNS2="   - DNS Alternativo (Ej: 8.8.4.4): "
...
echo.
echo --------------------------------------------------------
echo Presiona ENTER para aplicar o CIERRA la ventana si hay error.
pause >nul

echo.
echo [4/5] Aplicando configuracion IP estatica...
netsh interface ip set address name="%ADAPTADOR%" static %IP% %MASCARA% %PUERTA%

echo [5/5] Estableciendo servidores DNS...
netsh interface ip set dns name="%ADAPTADOR%" static %DNS1%
netsh interface ip add dns name="%ADAPTADOR%" %DNS2% index=2

echo.
echo --------------------------------------------------------
echo PROCESO FINALIZADO. Probando conexion a Google...
ping 8.8.8.8 -n 2
echo.
pause
goto :MENU_PRINCIPAL
